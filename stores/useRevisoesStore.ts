...
export const useRevisoesStore = create<RevisoesStore>((set, get) => ({
  ...
  fetchRevisoes: async (studyPlanId: string) => {
    ...
    try {
      ...
      // Calcular status de atraso imediatamente para evitar update extra
      const hoje = startOfDay(new Date());
      const revisoesProcessadas = revisoes.map(r => {
        const dataPrevista = startOfDay(new Date(r.data_prevista));
        if (r.status === 'pendente' && isBefore(dataPrevista, hoje)) {
          return { ...r, status: 'atrasada' as const };
        }
        return r;
      });

      set({ revisoes: revisoesProcessadas, loading: false });
    } catch (e) {
  ...
  removeRevisao: async (id) => {
    console.log('[useRevisoesStore] removeRevisao chamado com id:', id);
    try {
      await deleteRevisao(id);
      console.log('[useRevisoesStore] deleteRevisao completado com sucesso');
      set(state => ({ revisoes: state.revisoes.filter(rev => rev.id !== id) }));
      toast.success("Revisão excluída com sucesso!");
    } catch (e) {
      console.error('[useRevisoesStore] Erro ao remover revisão:', e);
  ...
  concluirRevisao: async ({ id, resultado, novaDificuldade }) => {
    ...
    // Atualizar revisão com status 'concluida' E data_conclusao para que o progresso diário funcione
    await get().updateRevisao(id, {
      status: 'concluida',
      dificuldade: mappedNovaDificuldade ?? revisaoOriginal.dificuldade,
