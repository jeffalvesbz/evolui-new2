...
const Etapa3: React.FC<{ formMethods: any }> = ({ formMethods }) => {
    ...
    // Move useMemo outside of the map function to comply with React's Rules of Hooks
    const disciplinasMap: Map<string, string> = useMemo(
        () => new Map(useDisciplinasStore.getState().disciplinas.map(d => [d.id, d.nome])),
        []
    );
...
const CriarCicloModal: React.FC = () => {
    ...
    const handleNext = async () => {
        ...
        if (etapa === 1) {
            ...
            // Criar um mapa de sessões por matéria
            const sessoesPorMateria = new Map<string, Omit<SessaoCiclo, 'id' | 'ordem'>[]>();
            ...
            materias.forEach(materia => {
                ...
                const sessoesDaMateria: Omit<SessaoCiclo, 'id' | 'ordem'>[] = [];

                for (let i = 0; i < numSessoes; i++) {
                    sessoesDaMateria.push({ disciplina_id: materia.id, tempo_previsto: tempoSessaoMinutos * 60 });
                }
                if (resto > 0) {
                    sessoesDaMateria.push({ disciplina_id: materia.id, tempo_previsto: resto * 60 });
                }

                sessoesPorMateria.set(materia.id, sessoesDaMateria);
            });

            // Distribuir as sessões de forma inteligente para evitar repetições consecutivas
            const sessoesDistribuidas: Omit<SessaoCiclo, 'id' | 'ordem'>[] = [];
            const ultimaPosicaoPorMateria = new Map<string, number>();

            while (sessoesPorMateria.size > 0) {
                // Encontrar a matéria com mais sessões restantes que está mais distante da última posição
                let melhorMateria: string | null = null;
                let melhorDistancia = -1;
                let maiorQuantidade = 0;

                for (const [materiaId, sessoes] of sessoesPorMateria.entries()) {
                    const ultimaPosicao = ultimaPosicaoPorMateria.get(materiaId) ?? -Infinity;
                    const distancia = sessoesDistribuidas.length - ultimaPosicao;
                    const quantidade = sessoes.length;

                    // Priorizar matérias com mais sessões, mas evitar colocar a mesma matéria consecutivamente
                    if (quantidade > maiorQuantidade || (quantidade === maiorQuantidade && distancia > melhorDistancia)) {
                        melhorMateria = materiaId;
                ...
                if (melhorMateria) {
                    const sessoes = sessoesPorMateria.get(melhorMateria)!;
                    sessoesDistribuidas.push(sessoes.shift()!);
                    ultimaPosicaoPorMateria.set(melhorMateria, sessoesDistribuidas.length - 1);

                    if (sessoes.length === 0) {
                        sessoesPorMateria.delete(melhorMateria);
                    }
                }
            }

            setValue('sessoesGeradas', sessoesDistribuidas);
        }
