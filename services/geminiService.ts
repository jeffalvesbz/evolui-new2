...
- Mantenha tom técnico de corretor oficial, mas seja pedagógico e encorajador`;
    ...
    const avaliacaoDetalhadaProcessada = resultado.avaliacaoDetalhada?.map((item: any, index: number) => {
    ...
    if (notaFinalCalculadaFinal > notaMaxima) {
        console.warn(`Nota final (${notaFinalCalculadaFinal}) excede o máximo (${notaMaxima}), ajustando...`);
        notaFinalCalculadaFinal = notaMaxima;
    }
...
export const deleteRedacao = async (redacaoId: string): Promise<void> => {
    const { error } = await supabase.from('redacoes_corrigidas').delete().eq('id', redacaoId);
    if (error) throw error;
};
